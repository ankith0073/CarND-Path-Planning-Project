	...
	h.onMessage([&ref_vel, &lane, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy]
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2') {
			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					for ( int i = 0; i < sensor_fusion.size(); i++ ) {
						...
						int car_lane = -1;
						// the current d information from localization is used to determine which lane the car is in right now
						if ( d > 0 && d < 4 ) {
							car_lane = 0;
						} else if ( d > 4 && d < 8 ) {
							car_lane = 1;
			...
			// follows closely the question and answer session from https://www.youtube.com/watch?v=7sI3VHFPP0w path planning project
					...
					//Data from sensor fusion is used to decide waht is the itended behavior which needs to be performed!
                    //Behaviors include changing lanes, or keep lane and accelerate and maintain velocity
					double speed_diff = 0;
                    ...
                    //get the y position from the target_x information
